...
intervalos_idade <- cut(dados_cluster$Diagnosis.Age, breaks = c(0, 50, 60, 70, 80, 90, Inf),
                        labels = c("0-50", "51-60", "61-70", "71-80", "81-90", "91+"))

# Adicionar os intervalos de idade como uma nova coluna aos dados_cluster
dados_cluster$Intervalos_Idade <- intervalos_idade

# Reordenar os dados_cluster com base nos intervalos de idade
dados_cluster <- dados_cluster[order(dados_cluster$Intervalos_Idade), ]

# Remover a coluna de intervalos de idade após reordenar
dados_cluster <- dados_cluster[, -ncol(dados_cluster)]
...
dados_teste$Diagnosis.Age <- as.numeric(dados_teste$Diagnosis.Age)

# Ajustar o modelo de regressão linear
modelo_regressao <- lm(Overall.Survival..Months. ~ Diagnosis.Age, data = dados_treino)

# Ajustar o modelo de árvore de decisão
modelo_arvore <- rpart(Overall.Survival..Months. ~ Diagnosis.Age, data = dados_treino)

# Prever com ambos os modelos nos dados de teste
predicoes_regressao <- predict(modelo_regressao, newdata = dados_teste)
predicoes_arvore <- predict(modelo_arvore, newdata = dados_teste)

# Calcular métricas de avaliação para o modelo de regressão linear
auc_regressao <- roc(dados_teste$Overall.Survival..Months., predicoes_regressao)
auc_regressao <- auc(auc_regressao)

# Calcular métricas de avaliação para o modelo de árvore de decisão
auc_arvore <- roc(dados_teste$Overall.Survival..Months., predicoes_arvore)
auc_arvore <- auc(auc_arvore)

# Comparar os resultados
print(paste("AUC do modelo de regressão linear:", auc_regressao))
print(paste("AUC do modelo de árvore de decisão:", auc_arvore))
