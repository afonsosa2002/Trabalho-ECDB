...
# análise preditiva/ aprendizagem máquina

dados <- dados_patient[, c("Diagnosis.Age", "Overall.Survival..Months.")]

set.seed(123) # Define a semente para reproducibilidade
indices_treino <- sample(1:nrow(dados), 0.8 * nrow(dados))
dados_treino <- dados[indices_treino, ]
dados_teste <- dados[-indices_treino, ]


#Remover linhas com valores NA
dados_treino <- dados_treino[complete.cases(dados_treino), ]

dados_treino$Diagnosis.Age <- as.numeric(dados_treino$Diagnosis.Age)
dados_treino$Overall.Survival..Months. <- as.numeric(dados_treino$Overall.Survival..Months.)
